    ...
    <div style={{ position: "fixed", inset: 0, background: "#000c", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: C.surface, border: `2px solid ${C.purple}`, borderRadius: 16, padding: 32, width: 480 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onConfirmar} style={{ flex: 1, background: C.purple, color: "#fff", border: "none", borderRadius: 9, padding: "12px", fontWeight: 800, fontSize: 14, cursor: "pointer" }}>
  ...
  return (
    <div>
      ...
      <SectionHeader
        ...
        action={<button style={{ background: C.accent, color: "#fff", border: "none", borderRadius: 8, padding: "8px 18px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>+ Nuevo Presupuesto</button>}
      />

      {listos.length > 0 && (
        <div style={{ background: C.purpleDim, border: `1px solid ${C.purple}44`, borderRadius: 10, padding: "12px 16px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
          ...
          <button onClick={() => setFiltroEstado("READY_FOR_INVOICING")} style={{ background: C.purple, color: "#fff", border: "none", borderRadius: 7, padding: "6px 14px", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>Ver todos</button>
        </div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        {filtrados.map((q: any) => {
          ...
          return (
            <div key={q.id} style={{ display: "grid", gridTemplateColumns: "70px 1fr 130px 90px 110px 150px 130px", gap: 8, padding: "12px 14px", background: C.card, borderRadius: 8, border: `1px solid ${q.status === "READY_FOR_INVOICING" ? C.purple + "44" : C.border}`, alignItems: "center" }}>
              ...
              {q.status === "READY_FOR_INVOICING" ? (
                <button onClick={() => setExportQ(q)} style={{ background: C.purple, color: "#fff", border: "none", borderRadius: 7, padding: "6px 12px", fontWeight: 700, fontSize: 11, cursor: "pointer" }}>⬇ Exportar a Presea</button>
              ) : (
