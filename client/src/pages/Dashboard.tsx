          ...
          {repStats.map((rep: any, i: number) => (
            <div key={rep.id} style={{ marginBottom: 16 }}>
              ...
              <div style={{ height: 4, background: C.border, borderRadius: 2 }}>
                <div style={{ width: `${(rep.monto / maxMonto) * 100}%`, height: "100%", background: rep.color, borderRadius: 2 }} />
              </div>
            </div>
          ))}
        </div>
              ...
              return (
                <div key={lr.reason} style={{ marginBottom: 10 }}>
                  ...
                  <div style={{ height: 5, background: C.border, borderRadius: 2 }}>
                    <div style={{ width: `${(lr.amount / totalLoss) * 100}%`, height: "100%", background: C.red, borderRadius: 2 }} />
                  </div>
                </div>
              );
            })}
            ...
            {sectorStats.map((ss: any) => {
              ...
              return (
                <div key={ss.sector} style={{ marginBottom: 10 }}>
                  ...
                  <div style={{ height: 5, background: C.border, borderRadius: 2 }}>
                    <div style={{ width: `${(ss.amount / maxSector) * 100}%`, height: "100%", background: C.green, borderRadius: 2 }} />
                  </div>
                </div>
              );
            })}
